...
1. `/agents` → Running agents (list and view picker)
2. FleetView Enter → view picker (not on the fleet list itself)
...
- Use both methods in `/agents` and inside the overlay. FleetView Enter opens the picker; the fleet list does not bind `t` `i` `p` `o`.
...
- Key scenarios:
  ...
  2. FleetView on an agent row → Enter → arrow to Info → Enter, or press `i` in the picker.
...
- As a user I want FleetView Enter to open the same view picker as `/agents`.
...
2. After the user picks an agent in `/agents` → Running agents, the system must open the view picker. The running-agent list must accept `t` `i` `p` `o` on the highlighted row and skip the picker. The list footer must show `t/i/p/o`. FleetView does not bind those keys on the list itself — Enter opens the picker; other letters leave the list.
...
7. When FleetView is active, `t` `i` `p` `o` must not open an overlay. They leave the list like any other letter.
8. FleetView Enter on an agent row must open the view picker.
...
26. FleetView MUST NOT consume `t` `i` `p` `o` on the list. Those keys open views only inside the picker and the overlay.
...
- FleetView must not consume `t` `i` `p` `o` on the list. Those letters leave the list and type into the editor, as any other key does.
...
- Manual: FleetView Enter on an agent row opens the picker. `p` on the list does not open Prompt.
- Manual: Prompt text equals the Agent-tool `prompt` when `inherit_context` is on.
- Tests: picker and overlay consume `t` `i` `p` `o`. Composer does not switch views. FleetView letter keys leave the list.
...
- FleetView list is active: `t` `i` `p` `o` leave the list and do not open an overlay.
...
- FleetView: picker on Enter. No view keys on the list itself.
...
- Given FleetView is active and an agent row is selected, when I press `p`, then no overlay opens and the list is no longer active.
