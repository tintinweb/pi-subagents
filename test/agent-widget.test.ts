...
import { type AgentActivity, AgentWidget, buildInvocationTags, fgPreservingNestedStyles, formatSessionTokens, prepareModelIdForDisplay } from "../src/ui/agent-widget.js";

describe("prepareModelIdForDisplay", () => {
  it("keeps model IDs on one printable terminal line", () => {
    expect(prepareModelIdForDisplay("openai\u001b[2J\n/gpt-5.6-sol")).toBe("openai /gpt-5.6-sol");
  });
});

describe("buildInvocationTags", () => {
  it("keeps the effective model ID with its model-dependent thinking level", () => {
