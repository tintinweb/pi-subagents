...
### Added
- Configurable worktree checkout timeouts via `worktreeTimeoutSeconds` in global/project `subagents.json`, including an `auto` strategy that scales with tracked file count and a `/agents` → Settings control.
- Failed or timed-out worktree creation now attempts to terminate the Git process tree and remove partial worktree registrations/files.
