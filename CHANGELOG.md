...
### Added
- **Project custom agents can now live in `.agents/agents/<name>.md`.** Custom agent discovery now loads global agents first, legacy project agents from `.pi/agents/` second, and preferred project agents from `.agents/agents/` last so the preferred project location wins on name conflicts. The `/agents` create/eject/disable flows now write new project definitions to `.agents/agents/` while still editing, deleting, enabling, or disabling existing legacy `.pi/agents/` definitions. Tool descriptions, schema help, examples, and README/security docs advertise the preferred path while retaining the legacy path for compatibility.
