...
- **FleetView** — Claude Code-style navigable list of `main` + every running subagent rendered below the editor (earliest-launched first). Press `↓` (or `←`) at an empty prompt to jump in, `↑`/`↓` to move the selection, `Enter` to pick Transcript / Info / Prompt / Output (arrows + Enter, or `t` `i` `p` `o`), or press `t` `i` `p` `o` on an agent row to open that view at once. `Esc` returns. Finished agents linger briefly before dropping out, and a viewer stays open through completion so you can read the final output. Toggle via `/agents → Settings → Fleet view`
- **Conversation viewer** — select any agent in `/agents` to pick a view, then open a live overlay: **Transcript** (full conversation, auto-follows new content, scroll up to pause), **Info** (model, thinking, cost, tools, elapsed and wall-clock time), **Prompt** (the spawn prompt only), **Output** (final result, or `No output yet.`). Inside the overlay, `t` `i` `p` `o` switch views without closing it. Steer a running agent inline by pressing `Enter` to open a composer, typing, then `Enter` to send (`Esc` or an empty submit returns) — the message appears as a user message and redirects the agent after its current tool. Stop a still-running agent by pressing `x` (then `x` again to confirm) — both work for background agents too. Assistant text renders as Markdown; `m` cycles that between off, assistant-only and everything (see [Viewer markdown](#persistent-settings)). Spec: [Agent viewer modes](docs/agent-viewer-modes.md)
...
- **Running agents** — pick an agent, then pick Transcript / Info / Prompt / Output. Enter opens the view picker; `t` `i` `p` `o` skip it and open that view (same shortcut as FleetView). The list footer shows `t/i/p/o`. Transcript is the live conversation viewer. Info is model, thinking, cost, tools, and times (no turn count). Prompt is the spawn prompt only. Output is the final result, or `No output yet.` Inside the overlay, `t` `i` `p` `o` switch views. While the agent is still running, press `Enter` to open the steering composer, then `Enter` again to send a message that redirects the agent (same mechanism as the `steer_subagent` tool; `Esc` or an empty submit returns), or press `x` (then `x` again to confirm) to stop/abort it — including **background** agents, which a global Esc can't unambiguously target (Esc still stops a blocking foreground `Agent` call). A stopped agent reports its partial output flagged as incomplete, not as a completion. `m` cycles how much of the Transcript view renders as Markdown — see [Viewer markdown](#persistent-settings).
...
docs/                 # Long-form guides (shipped to npm; README links out to them)
  ...
  agent-viewer-modes.md # Transcript / Info / Prompt / Output overlay spec
examples/
...
src/
  ...
  ui/
    ...
    conversation-viewer.ts # Agent overlay: Transcript / Info / Prompt / Output
    agent-views.ts        # View picker, view keys, Info-line builder
