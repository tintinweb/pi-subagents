...
| `session_file` | — | Optional explicit session JSONL file. Implies persistence; relative paths resolve from the requested agent cwd before worktree isolation is applied; existing files are opened and appended/resumed instead of overwritten. One active run per file: concurrent writers are unsupported |
...
| `session_file` | string | no | Explicit session JSONL file. Implies persistence; relative paths resolve from the requested agent cwd before worktree isolation is applied; existing files are opened and appended/resumed. Do not reuse one file across concurrent runs |
