...
| `extensions`        | omitted        | Which extensions load (loader-level allowlist). `true` = all, `false` = none. Bare names match case-insensitive canonical extension names; paths are resolved and match only that exact extension resource. Omitted falls back to the global `defaultExtensions` setting, then all. Excluded extensions do not load, bind, or register tools |
