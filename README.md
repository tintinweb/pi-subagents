...
- **Running agents** — select one to pick Session / Prompt / Info / Output (arrows + Enter, or `s` `p` `i` `o`). Session is the live conversation viewer. Prompt is the spawn prompt only. Info is model, thinking, cost, tools, and times (no turn count). Output is the final result, or `No output yet.` Inside the overlay, `s` `p` `i` `o` switch views. While the agent is still running, press `Enter` to open the steering composer, then `Enter` again to send a message that redirects the agent (same mechanism as the `steer_subagent` tool; `Esc` or an empty submit returns), or press `x` (then `x` again to confirm) to stop/abort it — including **background** agents, which a global Esc can't unambiguously target (Esc still stops a blocking foreground `Agent` call). A stopped agent reports its partial output flagged as incomplete, not as a completion. `m` cycles how much of the Session transcript renders as Markdown — see [Viewer markdown](#persistent-settings).
...
docs/                 # Long-form guides (shipped to npm; README links out to them)
  ...
  agent-viewer-modes.md # Session / Prompt / Info / Output overlay spec
examples/
...
src/
  ...
  ui/
    ...
    conversation-viewer.ts # Agent overlay: Session / Prompt / Info / Output
