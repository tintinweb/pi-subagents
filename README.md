...
- **Session resume** — pick up where an agent left off, preserving full conversation context. Persisted sessions remain resumable by ID or handle after the in-memory record is evicted. Resumes detached by default and notifies you on completion, just like a fresh spawn; pass `run_in_background: false` to block and get the result inline
